...
from bokeh.models import (BasicTicker, ColorBar, ColumnDataSource, CustomJS,
                          FreehandDrawTool, HoverTool, LabelSet,
                          LinearColorMapper, LogColorMapper, PolyDrawTool,
                          Select)
...
class Plotter():
    ...
    def save_matplotlib_figure(self,suffix,bg_transparent=True,extension = 'png'):
        if extension == 'png':
        ...
        if self.neptune_experiment!=None:
            self.neptune_experiment[extension+"/"+self.fname+suffix].upload(self.get_full_path(extension,suffix))
            #self.neptune_experiment.sync()
...
class GeneralPlotter(Plotter):
    def __init__(self, params, neptune_experiment=None):
        ...
        default_params={
            ...
            "markers": None, # TODO: markers
            "fname": "general_plot",
            "dirname": "",
            "line45_color": None, #None to turn off line45
            "legend":{
                "labels": [],
                "location": None # "top_left", "top_right", "bottom_left", "bottom_right", None
            },
            "baselines":{
                "labels": [],
                "values": [],
        ...
        if len(self.Ys) > 0 and not isinstance(self.Ys[0], Iterable):
            self.Ys = [self.Ys]
        self.Ys = [list(y) for y in self.Ys]
        
        for label in ('xlabel', 'ylabel'):
            setattr(self, label, getattr(self, label).replace('_', ' '))
        self.legend['labels'] = [label.replace('_', ' ') for label in self.legend['labels']]
        
        if len(self.legend["labels"]) + len(self.baselines["labels"]) + len(self.histogram["labels"]) == 0:
            self.legend["location"] = None

        if self.legend['location'] is None:
            self.legend['location'] = 'top_right' if np.argmax(self.Ys[0]) < np.argmin(self.Ys[0]) else 'bottom_right'
    ...
    # matplotlib #
    ##############
    def export_matplotlib(self, color_settings={"suffix":"", "grid_color":"0.9", "face_color":"white", "line_color": "black", "bg_transparent":False}, extension = 'png'):
        with plt.style.context((self.matplotlib["style"])):
            ...
            if self.legend["location"]!=None and (len(self.legend["labels"])>0 or len(self.baselines["labels"]) > 0 or len(self.histogram["labels"]) > 0):
                legend = ax.legend(loc=self.legend["location"].replace("top_","upper ").replace("bottom_","lower "))
            ...
            super().save_matplotlib_figure(color_settings["suffix"],color_settings["bg_transparent"], extension = extension)
...
class ScatterPlotter(Plotter):
    def __init__(self, params, neptune_experiment=None):
        ...
        if len(self.Xs) > 0 and not isinstance(self.Xs[0], Iterable):
            self.Xs = [self.Xs]
        self.Xs = [list(x) for x in self.Xs]
    ...
    # bokeh #
    #########
    def export_bokeh(self, heatmap=False, suffix="", theme_suffix="", palette=cc.CET_L18):
        p = super().init_bokeh_figure(theme_suffix) 
        ...
        for x,y,color,label in zip(self.Xs, self.Ys, self.colors, self.legend['labels']):
            if label!=None:
    ...
    # matplotlib #
    ##############
    def export_matplotlib(self, color_settings={"suffix":"", "grid_color":"0.9", "face_color":"white", "line_color": "black", "bg_transparent":False, "cmap":"cet_CET_L18"}, heatmap=False, suffix="", extension = 'png'):
        with plt.style.context((self.matplotlib["style"])):
            ax = super().init_matplotlib_figure()          
            ...
            super().save_matplotlib_figure(suffix+color_settings["suffix"],color_settings["bg_transparent"],extension = extension)
