...
class ReplaceValues(TransformWrapper):
    
    PARAMS = {'in_keyword': 'frequency',
              'out_keyword': 'weight',
              'replacements': {500: 1},
              'default': 0.5}
    
    def __init__(self, config_dict = PARAMS, *args, **kwargs):
        self.in_kw = config_dict['in_keyword']
        self.out_kw = config_dict['out_keyword']
        self.replace_dict = config_dict['replacements']
        self.default = config_dict['default']
    
    def transform(self, input):
        x = np.array(input[self.in_kw])
        y = x.copy() if self.default is None else self.default * np.ones_like(x)
        for k, v in self.replace_dict.items():
            y = np.where(x == k, v, y)
        input[self.out_kw] = y
        return input

class ModularShift(TransformWrapper):
    
    PARAMS = {'probability': 1.0, 'modulus': 1, 'minimum': -0.5, 'keyword': 'x0'}
    
    def __init__(self, config_dict, seed = None, *args, **kwargs):
        for k, v in config_dict.trim().items():
            setattr(self, k, v)
        self.rng = np.random.default_rng(seed = seed)
    
    def transform(self, input):
        if self.rng.binomial(1, self.probability):
            x = np.array(input[self.keyword])
            shift = self.rng.uniform(0, self.modulus, size = x.shape[0])
            x = x + np.moveaxis(np.tile(shift, (*x.shape[1:], 1)), -1, 0)
            input[self.keyword] = self.minimum + (x % self.modulus)
        return input
